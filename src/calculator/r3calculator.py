...
def r3_calculator(w: float, h: float, egg_form: int) -> float:
    """
    Calculate the radius r3 for a given width (w), height (h), and egg shape type (egg_form).
    ...
    r1: float = (h - w) / (2.0 if egg_form == 1 else 4.0)
    h2: float = h - r2
    r3: float = h
    diff: float = 1.0
    ...
    while abs(diff) > precision and iterations < max_iterations:
        ...
        offset_a: float = math.sqrt(square_term)
